...
export default function SystemMessage() {
  const [systemMessage, useSystemMessageForAllChats, setSystemMessage, setSystemMessageModalVisible] = useSettings((state) => [
  ...
  const [useForAllChats, setUseForAllChats] = useState(useSystemMessageForAllChats);
    ...
    >
      <form onSubmit={handleOnSubmit}>
        ...
        <div className='mt-2 flex items-center mb-4 justify-between border border-gray-200 rounded dark:border-gray-700 p-2'>
          <label htmlFor='default-checkbox' className='ml-2  font-medium  dark:text-gray-300'>
            Use for all chats
          </label>
          <label className='relative inline-flex items-center cursor-pointer'>
            <input type='checkbox' value='' checked={useForAllChats} onChange={(e) => setUseForAllChats(e.target.checked)} className='sr-only peer' />
            <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600"></div>
          </label>
        </div>
        <div className='flex justify-end mt-2'>
          <button className=' bg-teal-700 hover:bg-teal-900 text-white px-4 py-2 rounded mr-2' type='submit'>
            Save
          </button>
          <button className='bg-gray-200 hover:bg-gray-400 text-gray-700 px-4 py-2 rounded' onClick={() => setSystemMessageModalVisible(false)}>
            Cancel
          </button>
        </div>
      </form>
