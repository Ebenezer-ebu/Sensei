...
export default function useBot({ index, chat }: Props) {
  ...
  const [sendHistory, selectedModal, systemMessage, useForAllChats] = useSettings((state) => [
  ...
  }, [query, addChat, index, scrollToBottom, chat.content, chat.id, chat.type, sendHistory, selectedModal, systemMessage, useForAllChats]);
