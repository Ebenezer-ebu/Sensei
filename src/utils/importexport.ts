import useChat, { ChatMessageType, SettingsType, Theme, useSettings, useTheme } from '../store/store';
...
export function handleExportChats(): Promise<boolean> {
  return new Promise((resolve, reject) => {
    try {
      ...
      useChat.getState().chatHistory.forEach((c) => (backup.conversations[c] = JSON.parse(localStorage.getItem(c) as string)));
      const settingsClone: SettingsType['settings'] = JSON.parse(JSON.stringify(useSettings.getState().settings));
...
export function handleImportChats(file: File): Promise<Error | boolean> {
  ...
  if (file.type !== 'application/json') return Promise.reject(new Error('File is not a json file'));
  return new Promise((resolve, reject) => {
    try {
      ...
      reader.onload = function (e) {
        ...
        useChat.setState((prev) => ({
          chatHistory: getOnlyUniqueIds(Object.keys(chats.conversations).concat(prev.chatHistory)),
        }));
        ...
        localStorage.setItem('chatHistory', JSON.stringify(getOnlyUniqueIds(Object.keys(chats.conversations).concat(useChat.getState().chatHistory))));
        resolve(true);
      };
