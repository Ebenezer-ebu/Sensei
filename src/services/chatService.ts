...
export async function fetchResults(messages: Omit<ChatMessageType, 'id' | 'type'>[], modal: string, signal: AbortSignal, onData: (data: any) => void, onCompletion: () => void) {
  try {
    const response = await fetch(apiUrl, {
      ...
      body: JSON.stringify({
        ...
        stream: true,
        messages: messages,
        // max_tokens: 100, Just on here for check and limit
      }),
    });
...
export type ImageSize = '256x256' | '512x512' | '1024x1024' | '1280x720' | '1920x1080' | '1024x1024' | '1792x1024' | '1024x1792';
...
export async function generateImage(prompt: string, size: ImageSize, numberOfImages: number) {
  ...
  try {
    const response = await fetch(IMAGE_GENERATION_API_URL, {
      ...
      body: JSON.stringify({
        ...
        prompt: prompt,
        n: numberOfImages,
        size: useSettings.getState().settings.dalleImageSize[selectedModal as DallEImageModel],
      }),
    });
