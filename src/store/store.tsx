...
const useChat = create<ChatType>((set, get) => ({
  chats: [],
  chatHistory: localStorage.getItem('chatHistory') ? JSON.parse(localStorage.getItem('chatHistory') as string) : [],
  addChat: (chat, index) => {
    set(
  ...
  saveChats: () => {
    ...
    localStorage.setItem('chatHistory', JSON.stringify([...get().chatHistory, chat_id]));
    set(
      produce((state: ChatType) => {
...
export const months = ['Januray', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];
export const priority = ['Today', 'Previous 7 Days', 'Previous 30 Days', 'This month'].concat(months);

export const selectChatsHistory = (state: ChatType) => {
  const sortedData: Record<string, { title: string; id: string; month: string; month_id: number }[]> = {};
  state.chatHistory.forEach((chat_id) => {
    const { title, id, createdAt } = JSON.parse(localStorage.getItem(chat_id) as string);
